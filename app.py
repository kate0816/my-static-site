...
def index():
    if request.method == 'POST':
        ...
        expected_html = expected_df.head().to_html(classes='table table-striped')
        actual_html = actual_df.head().to_html(classes='table table-striped')
        comparison_html = comparison_result.head().to_html(classes='table table-striped')
...
def download_file():
    ...
    # 按照指定的欄位順序排序
    columns_order = ['物料號碼', '數量', '應到', '實到', '差異']
